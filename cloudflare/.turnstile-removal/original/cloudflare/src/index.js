...
export default {
    async fetch(request, env) {
        ...
        if (url.pathname === '/v1/auth/email/login' && request.method === 'POST') {
            ...
            if (!validPassword)
                return error('invalid_credentials', 401, request, env, 'Email or password is invalid')
        ...
        if (url.pathname === '/v1' || url.pathname.startsWith('/v1/')) {
            ...
            if (url.pathname === '/v1/raindrop' && request.method === 'POST') {
                ...
                const now = Date.now()
                const inserted = await env.DB.prepare('INSERT INTO bookmarks (user_id, url, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
                    .bind(session.user_id, bookmarkUrl, title, now, now).run()
                return json({
                    result: true,
                    item: { _id: String(inserted.meta.last_row_id), link: bookmarkUrl, title }
                }, 201, request, env)
            }
